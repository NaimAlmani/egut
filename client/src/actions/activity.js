...
export const getExpiredActivities = () => dispatch => {
  dispatch(startLoading());
  axiosInstance
    .get('/api/activity/expired')
    .then(res => {
      dispatch({
        type: GET_EXPIRED_ACTIVITIES,
        payload: res.data
      });
      dispatch(endLoading());
    })
...
export const addNewActivity = activity => dispatch => {
  ...
  formData.set('isWeekly', activity.isWeekly);
  formData.set('startDate', activity.startDate);
  formData.set('endDate', activity.endDate);
...
export const ActivateActivity = (activityID, isActive) => dispatch => {
  dispatch(startLoading());
  axiosInstance
    .post('/api/activity/activateactivity', {
      id: activityID,
      is_active: isActive
    })
    .then(res => {
...
export const deleteMember = (activityID, memberID) => dispatch => {
  dispatch(startLoading);
  axiosInstance
    .post('/api/activity/deletemember', {
      activity: activityID,
      member: memberID
    })
    .then(res => {
