...
export default function(state = initialState, action) {
	switch (action.type) {
		...
		case SELECT_CATEGORY:
			const selectedCat = state.categories.find((c) => c.id === action.payload.id);
			let newCats;
			if (action.payload.isSelect === true) {
				newCats = [ selectedCat, ...state.selectedCategories ];
			} else {
				newCats = state.selectedCategories.filter((c) => c.id !== action.payload.id);
			}
		...
		case SELECT_GROUP:
			...
			const newGActs = filterActivities(state.activities, state.selectedCategories, newGr, state.selectedDays);
			return {
				...state,
